...
class ModelModuleDQuickcheckout extends Model {
	...
	*/
	...
	function startDebug($message = '') {
		...
		//$this->logWrite($message. ' -  start ', $this->config->get('d_quickcheckout_status')); 
	}

	function endDebug($message) {
		...
		// выводим в выходной поток время генерации страницы
		 $this->logWrite( $message.'  - '.number_format($time, 4, '.', '')." секунд", $this->config->get('d_quickcheckout_status')); 
	}
    ...
    public function setStatistic($setting_id, $order_id, $data){
    	...
    	$this->db->query("INSERT INTO " . DB_PREFIX . "dqc_statistic 
			SET setting_id = '" . (int)$setting_id . "', 
				...
				data = '" . $this->db->escape(serialize($this->session->data['statistic'])) . "', 
				rating = 0,
				date_added = NOW(),
				date_modified = NOW()");
