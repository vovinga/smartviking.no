	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if (isset($seoreplacer['customtitles']) && ($seoreplacer['customtitles'])) {
				$this->db->query("UPDATE " . DB_PREFIX . "category_description SET custom_title = REPLACE (custom_title, '".$seoreplacer['replace']."', '".$seoreplacer['replacewith']."') WHERE language_id = '".$seoreplacer['language_id']."';");
				$this->db->query("UPDATE " . DB_PREFIX . "product_description SET custom_title = REPLACE (custom_title, '".$seoreplacer['replace']."', '".$seoreplacer['replacewith']."') WHERE language_id = '".$seoreplacer['language_id']."';");
				$this->db->query("UPDATE " . DB_PREFIX . "information_description SET custom_title = REPLACE (custom_title, '".$seoreplacer['replace']."', '".$seoreplacer['replacewith']."') WHERE language_id = '".$seoreplacer['language_id']."';");
			...
			$this->redirect($this->url->link('catalog/seoreplacer', 'token=' . $this->session->data['token'], 'SSL'));
		}
		...
		$this->data['cancel'] = $this->url->link('catalog/seoreplacer', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->data['action'] = $this->url->link('catalog/seoreplacer', 'token=' . $this->session->data['token'], 'SSL');
		
		$this->data['seoreplacer'] = array();
		
		if (isset($this->request->post['seoreplacer'])) {
			$this->data['seoreplacer'] = $this->request->post['seoreplacer'];
		} else {
			$this->data['seoreplacer'] = $this->config->get('seoreplacer');
		}
		...
		$this->template = 'catalog/seoreplacer.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
