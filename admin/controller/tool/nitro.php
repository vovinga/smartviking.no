...
class ControllerToolNitro extends Controller { 
    ...
    public function __construct($registry) {
        ...
        require_once DIR_SYSTEM . 'nitro/core/http_response_code.php';
    ...
    public function index() {
        ...
        if (!file_exists(dirname(DIR_SYSTEM) . '/vqmod/xml/b5b_admintheme_circloid.xml')) {
        ...
        $this->document->addStyle('view/javascript/nitro/bootstrap/css/bootstrap.min.css?version=' . $this->version);
        ...
        $this->data['pagespeedStoreUrl'] = (defined('HTTP_SERVER') && HTTP_SERVER != '') ? dirname(HTTP_SERVER) . '/' : false;
        $this->data['pagespeedSaveUrl'] = preg_replace("/^https?\:\/\//", "//", html_entity_decode($this->url->link("tool/nitro/savePagespeedResults", "token=" . $this->session->data["token"])));
        $this->data['pagespeedApiKey'] = !empty($persistence['Nitro']['GooglePageSpeedApiKey']) ? $persistence['Nitro']['GooglePageSpeedApiKey'] : '';
        ...
        $this->data['nitroData'] = $persistence;
        ...
        if ($cannotUseMinify) {
            ...
            $this->data['nitroData'] = $persistence_temp;
        }
        ...
        if (empty($persistence['Nitro']['ImageCache']['DimensionAutoDetect'])) {
            ...
            if (!empty($dimensionOverrides)) {
                ...
                $this->data['nitroData']['Nitro']['DimensionOverride'] = $dimensionOverrides;
            }
        }

        if (!empty($this->data['nitroData']['Nitro']['DimensionOverride'])) {
            $dimensionCategoryTitles = $this->get_category_heading_titles($this->data['nitroData']['Nitro']['DimensionOverride']);
        } else {
            $dimensionCategoryTitles = $this->get_category_heading_titles(array());
        }

        $this->data['dimensionCategoryTitles'] = $dimensionCategoryTitles;
        
        $this->data['inMaintenanceMode'] = inMaintenanceMode();
        ...
        $this->data['has_base_css_cache'] = is_dir(NITRO_BASE_CSS_FOLDER) ? count(scandir(NITRO_BASE_CSS_FOLDER)) > 2 : false;

        $this->data["image_dimensions_url"] = html_entity_decode($this->url->link("tool/nitro/get_image_dimensions", "token=" . $this->session->data["token"], 'SSL'));
        ...
        if (isset($this->session->data['success'])) {
            $this->data['success'] = is_array($this->session->data['success']) ? '<strong>Success</strong><br/><br/>' . implode('<br/>', $this->session->data['success']) : $this->session->data['success'];
