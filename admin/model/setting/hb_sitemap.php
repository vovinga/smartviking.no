...
class ModelSettingHbSitemap extends Model {
	...
	public function productDataView($language_id){
		$sql = "select `a`.`product_id` AS `product_id`,`b`.`name` AS `name`, `b`.`language_id` AS `language_id`,`a`.`model` AS `model` from (`" . DB_PREFIX . "product` `a` join `" . DB_PREFIX . "product_description` `b`) where (`a`.`product_id` = `b`.`product_id`) and b.language_id = $language_id";
		$query = $this->db->query($sql);
		return $query->rows;
	}
	
	public function categoryDataView($language_id){
		$sql = "SELECT a.category_id, b.name, b.language_id  FROM `" . DB_PREFIX . "category` a, " . DB_PREFIX . "category_description b WHERE a.category_id = b.category_id and  b.language_id = $language_id";
		$query = $this->db->query($sql);
		return $query->rows;
	}
	...
	public function getUrlkeyword($type, $id){
		$result = $this->db->query("SELECT * FROM `" . DB_PREFIX . "url_alias` where `query` = '".$type."=".$id."' AND language_id = '".$language_id."'");
	...
	public function getCountAlias($page_type){
		$results = $this->db->query("SELECT count(*) as count FROM `" . DB_PREFIX . "url_alias` WHERE query like ('".$page_type."=%') AND language_id = '".$language_id."'");
		return $results->row['count'];
	}
