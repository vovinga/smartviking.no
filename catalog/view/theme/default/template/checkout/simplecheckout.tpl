...
<?php } ?>
    ...
    <!-- simplecheckout form -->
        <?php 
            $replace = array(
                ...
                '{/right_column}' => '</div>',
                '{customer}'      => '<div '.(($this->customer->isLogged()) ? 'style="margin-top:-15px;"' : '').' class="simplecheckout-block'.($simple_customer_hide_if_logged ? ' simplecheckout-skip' : '').'" id="simplecheckout_customer"'.($simple_customer_hide_if_logged ? ' style="display:none;"' : ' ').'>'. $simplecheckout_customer .'</div>',
    ...
    <div class="simplecheckout-button-block buttons" id="buttons" <?php if ($block_order) { ?>style="display:none;"<?php } ?>>
        <div class="simplecheckout-button-right">
            <?php if ($simple_common_view_agreement_checkbox) { ?><label style="margin-right:15px;"><input type="checkbox" id="agree" name="agree" value="1" <?php if ($agree == 1) { ?>checked="checked"<?php } ?> /><?php echo $text_agree; ?></label>&nbsp;<?php } ?><a class="button btn" onclick="simplecheckout_submit();" id="simplecheckout_button_confirm"><span><?php echo $button_order; ?></span></a>
        </div>
